...
module.exports.manager = manager;
  ...
  // ==================== 迅雷短信验证码登录（代理 G-Box API）====================
  const GBOX_URL = 'http://192.168.3.16:4567';
  let _gboxToken = null;
  let _gboxTokenExpire = 0;

  function getGboxToken() {
    if (_gboxToken && Date.now() < _gboxTokenExpire) return Promise.resolve(_gboxToken);
    return new Promise((resolve, reject) => {
      const http = require('http');
      const postData = JSON.stringify({ username: 'admin', password: 'admin' });
      const req = http.request({
        hostname: '192.168.3.16', port: 4567,
        path: '/api/accounts/login', method: 'POST',
        ...
        res.on('end', () => {
          try {
            ...
            if (json.token) {
              _gboxToken = json.token;
              _gboxTokenExpire = Date.now() + 3600000;
              resolve(json.token);
            } else { reject(new Error('G-Box登录失败')); }
          } catch (e) { reject(e); }
        });
      });
      req.on('error', reject);
      req.setTimeout(10000, () => { req.destroy(); reject(new Error('G-Box连接超时')); });
      req.write(postData);
      req.end();
    });
  }

  function gboxApi(path, method, body) {
    return getGboxToken().then(token => new Promise((resolve, reject) => {
      const http = require('http');
      const postData = body ? JSON.stringify(body) : null;
      const headers = { 'X-ACCESS-TOKEN': token };
      if (postData) { headers['Content-Type'] = 'application/json'; headers['Content-Length'] = Buffer.byteLength(postData); }
      const req = http.request({
        hostname: '192.168.3.16', port: 4567,
        path: path, method: method, headers,
      }, (res) => {
        let data = '';
        res.on('data', c => data += c);
        res.on('end', () => {
      ...
      req.on('error', reject);
      req.setTimeout(15000, () => { req.destroy(); reject(new Error('G-Box API超时')); });
  ...
  // 发送迅雷短信验证码
  router.post('/xunlei/sms/send', async (req, res) => {
    try {
      const { mobile } = req.body || {};
      if (!mobile || !/^1\d{10}$/.test(mobile)) {
        return res.json({ ok: false, error: '请输入有效的11位手机号' });
      }
      const result = await gboxApi('/api/pan/accounts/thunder/sms/send-code', 'POST', {
        mobile, deviceId: '', creditKey: '', captchaToken: '',
      });
      if (result.sessionKey || result.deviceId) {
        return res.json({
          ok: true,
      ...
      return res.json({ ok: false, error: result.message || result.error || '发送失败' });
    } catch (e) {
  ...
  // 迅雷短信验证码登录
  router.post('/xunlei/sms/login', async (req, res) => {
    try {
      const { mobile, smsCode, sessionKey, name } = req.body || {};
      if (!smsCode) return res.json({ ok: false, error: '请输入短信验证码' });

      const result = await gboxApi('/api/pan/accounts/thunder/sms/login', 'POST', {
        sessionKey: sessionKey || '', smsCode,
      });

      const token = result.token || result.refreshToken || result.accessToken;
      if (token) {
        const accountName = name || ('我的迅雷云盘_' + (mobile || '').slice(-4));
        try {
          const account = await manager.createAccountWithCookie(
            'xunlei', accountName, token, result.refreshToken || token
          );
          return res.json({ ok: true, account });
        } catch (createErr) {
          // Even if account creation fails, return the token info
          return res.json({ ok: true, token: token, warning: '登录成功但账号创建失败: ' + createErr.message });
        }
      }
      return res.json({ ok: false, error: result.message || result.error || '登录失败' });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e.message });
    }
  });
...
router.delete('/accounts/:id', requireManager, async (req, res) => {
