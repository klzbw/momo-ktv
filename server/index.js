...
app.get('/api/songs/:id/sep-info', (req, res) => {
  ...
  if (song.source_root === 'netktv-mkv') {
    ...
    // 302直连模式（默认）：客户端直接访问115 CDN，不占NAS带宽和容量
    // 仅当客户端明确请求 ?proxy=1 时才走服务端代理（兼容旧客户端/UA校验失败场景）
    if (videoUrl && req.query.proxy === '1' && !videoUrl.includes('?proxy=')) {
