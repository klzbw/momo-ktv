...
def parse_cue(cue_path):
    ...
    for line in lines:
        ...
        if line.startswith('FILE '):
            ...
            current = {'no': int(m.group(1)) if m else len(tracks)+1, 'idx00': None, 'idx01': None}
        elif line.startswith('INDEX 00 ') and current:
            current['idx00'] = line.split()[-1]
        elif line.startswith('INDEX 01 ') and current:
            current['idx01'] = line.split()[-1]
    if current:
        tracks.append(current)
    return file_str, tracks

def fix_cue(src_dir, cue_name, titles, album_name):
    cue_path = os.path.join(src_dir, cue_name)
    file_str, tracks = parse_cue(cue_path)
    print(f'  {cue_name}: {len(tracks)}首, FILE={file_str}')
    shutil.copy2(cue_path, cue_path + '.bak2')
    lines = ['PERFORMER "群星"', f'TITLE "{album_name}"', file_str]
    for i, t in enumerate(tracks):
        title = titles[i] if i < len(titles) else f"音轨{t['no']:02d}"
        lines.append(f"  TRACK {t['no']:02d} AUDIO")
        lines.append(f'    TITLE "{title}"')
        lines.append('    PERFORMER "群星"')
        if t['idx00']:
            lines.append(f"    INDEX 00 {t['idx00']}")
        lines.append(f"    INDEX 01 {t['idx01']}")
