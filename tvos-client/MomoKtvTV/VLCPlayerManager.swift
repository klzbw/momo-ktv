...
class VLCPlayerManager: NSObject, ObservableObject {
    ...
    /// Cookie Jar 文件路径（library级别--http-cookie-jar用）
    /// --http-cookie传超长内容会致VLCLibrary崩溃,改用--http-cookie-jar指向文件
    private let cookieJarPath: String = {
        let tmp = NSTemporaryDirectory()
        return (tmp as NSString).appendingPathComponent("momo_vlc_cookies.txt")
    }()

    /// 网盘驱动 -> Cookie域名映射（Netscape格式需要domain）
    ...
    private func writeAllCookiesToJar(_ cookies: [String: String]) {
        ...
        do {
            ...
            log("CookieJar写入: \(cookies.count)个网盘, \(lines.count-2)条cookie")
        } catch {
    ...
    private func setupLibrary() {
        ...
        log("=== MomoKtvTV v2026.09.18-flac-absolute-url-fix ===")
