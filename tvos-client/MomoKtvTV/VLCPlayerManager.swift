...
class VLCPlayerManager: NSObject, ObservableObject {
    ...
    /// VLC Cookie Jar 文件路径（Netscape格式）
    /// 关键：--http-cookie直接传cookie内容会导致VLCLibrary初始化崩溃，
    /// 改用 --http-cookie-jar 指向文件，VLC初始化时读取文件中的cookie
    private lazy var cookieJarURL: URL = {
        let dir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return dir.appendingPathComponent("vlc_cookies.txt")
    }()

    /// 网盘驱动 -> Cookie域名映射（Netscape格式需要domain字段）
    private static let cookieDomainMap: [String: String] = [
        "quark": ".quark.cn",
        "pan115": ".115.com",
    ...
    /// 把所有网盘Cookie写入Netscape格式cookie jar文件
    /// 必须在首次setupLibrary之前调用，VLC只在library初始化时读取一次
    func writeCookieJar(cookies: [String: String]) {
        var lines = ["# Netscape HTTP Cookie File", "# Generated by MomoKtvTV", ""]
        var count = 0
        for (driver, cookieStr) in cookies {
            guard !cookieStr.isEmpty else { continue }
            let domain = VLCPlayerManager.cookieDomainMap[driver] ?? ".\(driver).com"
        ...
        do {
            try lines.joined(separator: "\n").write(to: cookieJarURL, atomically: true, encoding: .utf8)
            log("CookieJar写入: \(cookies.count)个网盘, \(count)条cookie")
        } catch {
            log("CookieJar写入失败: \(error)")
        }
    }

    /// 连接服务器后预加载所有网盘Cookie并写入cookie jar（纯网络+文件，不触碰VLC实例）
    func preloadCookieJar(baseURL: String) {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let cookies = json["cookies"] as? [String: String] {
                DispatchQueue.main.async {
                    self?.writeCookieJar(cookies: cookies)
                }
            }
        }.resume()
    }
    ...
    private func setupLibrary() {
        ...
        let options = [
            ...
            "--http-cookie-jar=\(cookieJarURL.path)",
            "--no-video-title-show",
        ...
        log("=== MomoKtvTV v2026.09.13-quark-fix6 ===")
        log("VLCLibrary初始化成功, cookie-jar=\(cookieJarURL.path), UA=\(VLCPlayerManager.cloud115UserAgent)")
    }
    #endif
