...
class VLCPlayerManager: NSObject, ObservableObject {
    ...
    /// Cookie Jar 文件路径（media级别:http-cookie-jar用，不碰library初始化）
    /// media级别:http-cookie对夸克CDN不生效(VLC不发送Cookie头),改用cookie文件
    private let cookieJarPath: String = {
        let tmp = NSTemporaryDirectory()
        return (tmp as NSString).appendingPathComponent("momo_vlc_cookies.txt")
    }()

    /// 把cookie写入Netscape格式文件(供:http-cookie-jar读取)
    private func writeCookieJarFile(cookie: String, domain: String) {
        var lines = ["# Netscape HTTP Cookie File", "# Generated by MomoKtvTV", ""]
        ...
        do {
            ...
            log("CookieJar文件写入: \(lines.count-2)条cookie, domain=\(domain)")
        } catch {
            log("CookieJar文件写入失败: \(error)")
        }
    }
    ...
    private func setupLibrary() {
        ...
        log("=== MomoKtvTV v2026.09.13-quark-fix9 ===")
    ...
    private func startPlayback(player: VLCMediaPlayer, url: URL, originalURL: URL, cloudCookie: String?) {
        ...
        if let cookie = cloudCookie, !cookie.isEmpty {
            ...
            if currentCloudDriver == "quark" {
                // 夸克CDN必须cookie,但media级别:http-cookie对夸克不生效(VLC不发送Cookie头)
                // 改用media级别:http-cookie-jar指向cookie文件,VLC播放时读取文件发送cookie
                // 只保留__puus(curl实测其余6项不需要),domain=.quark.cn
                let puus = cookie.components(separatedBy: "; ").first(where: { $0.hasPrefix("__puus=") }) ?? cookie
                writeCookieJarFile(cookie: puus, domain: ".quark.cn")
                media.addOption(":http-cookie-jar=\(cookieJarPath)")
                media.addOption(":http-cookie=\(puus)")
                log("夸克用media级http-cookie-jar(\(puus.count)字符), 兜底:http-cookie")
            } else {
                // 115等:保持media级别cookie(多项含分号用引号保护)
