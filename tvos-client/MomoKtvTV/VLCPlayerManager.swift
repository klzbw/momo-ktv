...
class VLCPlayerManager: NSObject, ObservableObject {
    ...
    private func startPlayback(player: VLCMediaPlayer, url: URL, originalURL: URL, cloudCookie: String?) {
        ...
        if let cookie = cloudCookie, !cookie.isEmpty {
            // 关键：VLC media级别:http-cookie对超长cookie不生效（夸克完整cookie 555字符会失败）
            // curl实测夸克CDN只需要__puus这一个cookie（其余6项不需要），精简到约255字符
            // 115保持完整cookie（约229字符，已验证可生效）
            var effectiveCookie = cookie
            if currentCloudDriver == "quark" {
                if let puus = cookie.components(separatedBy: "; ").first(where: { $0.hasPrefix("__puus=") }) {
