...
- 🤖 **AI人声分离**：DUAL双轨技术，任意歌曲实时分离人声/伴奏，遥控器丝滑调音
- 📝 **逐字歌词**：唱到哪个字哪个字变色，就像KTV里一样
- 🤖 **AI歌词**：没有歌词的歌曲，自动从网易/QQ/酷我在线补抓，AI生成精准逐字歌词
- 🎨 **动态背景**：纯音频歌曲自动显示水波纹、星空、极光等14种动态背景
- 🌟 **氛围特效**：掌声、干杯、喝彩、倒彩，祝福语全屏滚动
- 💾 **曲库管理**：自动扫描新歌，支持多个曲库目录
- ☁️ **网盘曲库**（开发中）：扫码添加115网盘/阿里云盘，指定文件夹自动串流播放
...
### 目标
在 momo-ktv 中内置网盘管理，**扫码添加 115 网盘 / 阿里云盘**，选择网盘里的 KTV 文件夹，自动扫描入库，通过直链串流播放。一次配置，永久使用，不需要额外部署 Alist/OpenList。

### 核心功能
1. **扫码登录**：管理后台显示 115/阿里云盘二维码，手机扫码授权
2. **文件夹选择**：浏览网盘目录，指定某个文件夹作为 KTV 曲库
3. **自动扫描**：后台定时扫描网盘文件夹，新增媒体文件自动入库
4. **STRM 串流**：播放时通过网盘直链串流，不占用本地存储空间
5. **缓存管理**：热门歌曲自动缓存到本地，LRU 淘汰，平衡速度与空间

### 设计文档
- 完整设计方案：[docs/DESIGN-cloud-drive.md](docs/DESIGN-cloud-drive.md)
...
### Q：没有NAS，用普通电脑能跑吗？
**A**：能！电脑安装Docker后，一条命令就能启动。看《docs/03-服务端部署.md》的「电脑Docker部署」章节。

### Q：歌曲从哪里来？
**A**：你可以从网上下载MKV格式的KTV歌曲（带MV和字幕），也可以用普通的MP3/FLAC音频文件。纯音频歌曲会自动显示动态背景和AI生成的歌词。网盘曲库功能上线后，直接扫码添加115/阿里云盘就能播放网盘里的歌曲。

### Q：AI人声分离是什么？
**A**：AI人声分离用 Demucs 模型把任意歌曲的人声和伴奏分离开，实现"消音"效果。分离后的文件可以上传到网盘，多设备共享。看《docs/DESIGN-cloud-drive.md》了解网盘共享方案。

### Q：手机麦克风有延迟怎么办？
**A**：确保手机和服务器连同一个WiFi，用5GHz频段，离路由器近一些。延迟通常在100-300毫秒，唱歌时基本感觉不到。

### Q：原唱/伴唱切换不了？
**A**：只有MKV文件有多个音轨才能切换。MP4可能有，MP3/FLAC等纯音频只有一个音轨。开启AI人声分离后，任意歌曲都能实现原唱/伴唱切换。

### Q：播放卡顿？
**A**：可能原因：网络带宽不够（用有线网络）、服务器性能太弱、歌曲码率太高。开启硬件转码（NVENC）可以大幅改善。看《docs/03-服务端部署.md》的「独立显卡支持」章节。
...
- **网盘集成**：115开放平台API + 阿里云盘开放平台API + 扫码OAuth
